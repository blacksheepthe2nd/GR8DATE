...
class Profile(models.Model):
    ...
    def has_private_access_to(self, other_profile):
        """Check if this profile has access to another profile's private photos"""
        if not other_profile or not self.user:
            return False
            
        # Check if there's an approved, non-expired access request
        access_request = PrivateAccessRequest.objects.filter(
            requester=self.user,
            target_user=other_profile.user,
            status='approved'
        ).first()
        
        return access_request and not access_request.is_expired
...
class Thread(models.Model):
    ...
    class Meta:  
        ...
        ordering = ['-updated_at']  # ADD THIS LINE
    def __str__(self) -> str:
...
class Message(models.Model):
    ...
    class Meta:
        ordering = ["created_at"]  # CHANGE FROM "-created_at" to "created_at"
        indexes = [
...
class PrivateAccessRequest(models.Model):
    ...
    def save(self, *args, **kwargs):
        # Set expiration to 72 hours when approved
    ...
    def is_expired(self):
        ...
        if self.status != self.Status.APPROVED or not self.expires_at:
            return False
