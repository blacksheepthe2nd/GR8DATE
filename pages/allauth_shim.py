...
def _allow_full_access(profile) -> bool:
    # STAFF/SUPERUSER FIX: Always allow full access for staff/superusers
    if profile.user.is_staff or profile.user.is_superuser:
        return True
    
    # Regular users need complete AND approved profiles
    is_complete = getattr(profile, "is_complete", False)
    is_approved = getattr(profile, "is_approved", False)
    return bool(is_complete and is_approved)
...
class ShimLoginView(View):
    ...
    def _target_url_name(self, user):
        ...
        # FIX: Changed 'preview' to 'preview_gate'
        return "dashboard" if _allow_full_access(prof) else "preview_gate"
