...
# Create mapping from CSV user_id to database ID
mapping = {}
for _, row in df.iterrows():
    ...
    try:
        ...
        mapping[csv_id] = db_user.id
        print(f"  CSV ID {csv_id} -> DB ID {db_user.id} (username: {username})")
    except User.DoesNotExist:
        print(f"  ❌ CSV ID {csv_id} - User '{username}' not found in database")

print(f"\nSuccessfully mapped {len(mapping)} users")

# Save mapping for the linking script
import json
with open('user_id_mapping.json', 'w') as f:
    json.dump(mapping, f, indent=2)
print("Saved mapping to user_id_mapping.json")

# Now create the fixed linking script
with open('link_with_mapping.py', 'w') as f:
    f.write('''import os
import django
import sys
import json
...
print("LINKING PROFILE IMAGES WITH MAPPING")
...
print("Created link_with_mapping.py")
